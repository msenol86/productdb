...
def _explode_and_fetch(type_label: str, fetch_function: typing.Callable[[typing.Any], typing.Any]) -> Response:
    result_object = type_api.explode_type_label(type_label.strip())
    if result_object:
...
def get_type(type_label: str):
    return _explode_and_fetch(type_label, type_api.get_type)


@app.route("/get_children_types/<type_label>")
def get_children_types(type_label: str):
    return _explode_and_fetch(type_label, type_api.get_children_types)


@app.route("/get_parent_type/<type_label>")
def get_parent_types(type_label: str):
    return _explode_and_fetch(type_label, type_api.get_parent_type)


@app.route("/new_type/<new_type_name>/extends/<parent_type_label>")
def new_type(new_type_name: str, parent_type_label: str):
    result_object = type_api.explode_type_label(parent_type_label)
    if result_object:
        fetch_json = type_api.extend_type(new_type_name, result_object["meta_name"], result_object["meta_version"])
        if fetch_json:
...
def add_fields(type_label: str):
    result_object = type_api.explode_type_label(type_label)
    if result_object:
        new_fields_list = json.loads(request.form["new_fields"])
        if new_fields_list:
            fetch_json = type_api.add_fields_to_type(result_object["meta_name"],
